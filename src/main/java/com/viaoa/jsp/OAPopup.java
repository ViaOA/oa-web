...
/**
 * turn an html element into a popup. if location is on viewport edge, then it will use the slide affect, else fade. use setVisible(b) to
 * show/hide.
 *
 * @author vvia
 */
public class OAPopup extends OAHtmlElement {
	...
	public String getScript() {
		...
		if (topx != null && OAString.isNumber(topx)) {
			topx += "px";
		}
		if (leftx != null && OAString.isNumber(leftx)) {
			leftx += "px";
		}
		if (bottom != null && OAString.isNumber(bottom)) {
			bottom += "px";
		}
		if (right != null && OAString.isNumber(right)) {
			right += "px";
		}

		if (OAString.isEmpty(topx) && OAString.isEmpty(bottom)) {
			topx = "0";
		}
		...
		String rightx = right;
		if (OAString.isEmpty(rightx) && OAString.isEmpty(leftx)) {
			rightx = "0";
		}
		...
		if (OAString.isNotEmpty(clickId)) {
			...
			sb.append("  if ($('#" + id + "').is(':visible')) $('#" + id + "').hide(" + type + ", 325); else $('#" + id + "').show(" + type
					+ ", 325);return false;}\n");
			sb.append(");\n");
		}
	...
	public String getAjaxScript() {
		...
		String js = sb.toString();
		if (lastAjaxSent2 != null && lastAjaxSent2.equals(js)) {
			js = null;
		} else {
			lastAjaxSent2 = js;
		}

...
/**
 * Servlet that allows REST API calls using HTTP for the OAModel.
 
GET ===========
1) read object using name and Id
http://localhost:8088/pi/api/{className/{id}
http://localhost:8088/pi/api/{className/{id}?pp={x}&pp1={x}&ppN={x}
    pp = PropertyPath(s) from the ClassName of reference data to send, instead of just the ref Id

http://localhost:8088/pi/api/client/1
http://localhost:8088/pi/api/clients?fromClass=company&fromId=1&fromPath=clients&pp=products
http://localhost:8088/pi/api/campaign/1?pp=product.client.company&pp=campaignLink.platformCampaign.platformVendor
 

2) get objects using a property path from a base object
http://localhost:8088/pi/api/{pluralClassName/fromClass={fromClass&fromId={id}?pp={x}&pp1={x}&ppN={x}
http://localhost:8088/pi/api/clients?fromClass=company&fromId=1&fromPath=clients&pp=products
 
3) query objects using where clause
http://localhost:8088/pi/api/{pluralClassName/query={whereClause}&pp1={x}&ppN={x}
http://localhost:8088/pi/api/clients?query=company.name like "AAA*"&pp=products 

4) filter support
http://localhost:8088/pi/api/campaigns?filter=unassigned

POST ===========
http://localhost:8088/pi/api/{className}
    create new object
PUT ===========
http://localhost:8088/pi/api/{className}[/{id}]
...
//qqqqqqqqq check basic auth
        ...
        if (OAString.isNotEmpty(s)) resp.addHeader("Access-Control-Allow-Origin", s); // ex:  "http://localhost:4200");
        try {
...
//qqqq            OAThreadLocalDelegate.setContext(this);
    ...
    /**
     * used to MAP an REST endpoint to an OAModel API call.     
     */
    ...
    /**
     *  Create custom REST endpoints for FE.
     *  todo:  move to controller to allow configuration.
     */
...
*/        
    ...
    protected int _process(final HttpServletRequest req, final HttpServletResponse resp) throws Exception {
        ...
        for ( ;enumx.hasMoreElements();  ) {
            ...
            if (ss == null) continue;
            for (String s : ss) {
                if (s != null) alPropertyPath.add(s);
            }
        }        
        ...
        for (Mapping map : alMapping) {
            if (!methodType.equalsIgnoreCase(map.methodType)) continue;
            if (pathInfo == null || map.pathInfo == null) continue;
            if (!pathInfo.equalsIgnoreCase(map.pathInfo)) continue;
            if (map.name != null) {
                String s = hmParam.get(map.name);
                if (map.value != null) {
                    if (!map.value.equalsIgnoreCase(s)) continue;
                }
                else {
                    if (!map.bDefault) continue;
                }
            }
        ...
        if (className != null) {
            ...
            if (clazz == null) hmClassName.get("detail"+className.toLowerCase());
            if (clazz == null) {
                clazz = hmClassPluralName.get(className.toLowerCase());
                if (clazz == null) hmClassPluralName.get("detail"+className.toLowerCase());
                bIsMany = true;
            }
        }
        ...
        if ("put".equalsIgnoreCase(methodType) && !bIsMany) {
            ...
            if (!bIsMany) {
                ...
                if (OAString.isEmpty(fromClass)) fromClass = hmParam.get("from");
                String fromId = hmParam.get("fromid");
                if (OAString.isEmpty(fromId)) fromId = hmParam.get("id");
                String fromPath = hmParam.get("frompath");
                if (OAString.isEmpty(fromPath)) {
                    fromPath = hmParam.get("path");
                    if (OAString.isEmpty(fromPath)) fromPath = className;
                }
                ...
                if (OAString.isNotEmpty(filterName)) {
                    ...
                    try {
                        if (values[0] == null) {
                            ...
                            if (params != null && params.length == 2 && params[0].getType().equals(Hub.class) && params[1].getType().equals(Hub.class)) {
                                constructors[0] = constructors[0].getDeclaringClass().getConstructor(null);
                            }
                        }
                        CustomHubFilter customHubFilter;
                        if (values[0] == null) customHubFilter = (CustomHubFilter) constructors[0].newInstance(null);
                        else customHubFilter = (CustomHubFilter) constructors[0].newInstance(values[0]);
                        ...
                        if (OAString.isNotEmpty(s)) {
                            if (OAString.isNotEmpty(query)) query += " AND ";
                            query += s;
                        }
                        
                        if (filterQuery == null) filterQuery = filter;
                        else filterQuery = new OAAndFilter(filterQuery, filter);
                    }
                    catch (Exception e) {
    ...
    protected int _process(final String methodName, final String url, String contentType, final Map<String, String[]> mapParam, final PrintWriter printWriter, final HttpServletRequest req, final HttpServletResponse resp) throws Exception {
        if (contentType == null) contentType = "";
        else contentType = contentType.toLowerCase();
...
//qqqqqqqqqq get User, could be null
        ...
        if ("GET".equalsIgnoreCase(methodName)) {
            ...
            if (contentType.indexOf("x-www-form-urlencoded") >= 0) bUseParams = true;
            else if (contentType.indexOf("multipart/form-data") >= 0) {
                bUseParams = true;
                bUseMultipart = true;
            }
            else bUseInputStream = true;
        }
        ...
        */
        ...
        if (req != null) {
            ...
            if (OAString.isNotEmpty(s)) {
                if (!s.startsWith("/")) ignoreName += "/";
                ignoreName += s;
            }
            if (OAString.isNotEmpty(s) && !s.endsWith("/")) ignoreName += "/";
        }
        if (ignoreName == null) ignoreName = "/vendor-api/";
...
//      printWriter.print(objResult + "");
    ...
    protected String getUserId(HttpServletRequest servletRequest) {
        ...
        try {
            ...
            if (jwt.equals(lastJwt)) return lastJwtUserId;
        }
        finally {
...
if (true) return;
        ...
        OAThreadLocalDelegate.setContext(this);
        ...
        if (user == null) {
            response.setHeader("WWW-Authenticate", "BASIC realm=\"OARestServlet\"");
            response.sendError(response.SC_UNAUTHORIZED);
        }
        else {
//qqqqqqqqqqqqqqqqq OAContext qqqqqqqqqqq  wont work well with many users            
    ...
    public OAUserAccess getUserAccess(Object user) {
        return null;
    }

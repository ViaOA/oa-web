 ...
 * @author vince
 */
public class OAInputImage extends InputImage implements OATableColumnInterface {
    private final OAUICommandController controlUI;

    public OAInputImage(String selector, Hub hub, OAUICommandController.Command command) {
        super(selector);
        controlUI = new OAUICommandController(hub, command) {
            @Override
            protected Object getManualObject() {
                return OAInputImage.this.getManualObject();
            }
            @Override
            protected boolean performCommand(Hub hub, OAObject obj) {
                if (command == Command.OtherUsesAO 
                    ...
                    return OAInputImage.this.performCommand(obj);
                }
                else { 

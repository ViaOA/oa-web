...
*/            
    ...
    /**
     * Get the form for the current request page.
     */
    public OAForm getRequestForm(String id) {
        OAForm form = getForm(id);
        if (form != null) {
            HttpServletRequest req = getRequest();
            if (req != null) {
                if (req.getParameterMap().containsKey("oadebug")) {
                    form.setDebug(!form.getDebug());
                }
                if (req.getParameterMap().containsKey("oareset")) {
                    return null;
                }
            }
            setCurrentForm(form);

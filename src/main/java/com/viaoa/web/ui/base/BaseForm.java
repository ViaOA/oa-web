...
import com.viaoa.web.ui.OAForm;
import com.viaoa.web.ui.OAJspDelegate;
import com.viaoa.web.ui.OATextField;
 ...
 * @author vvia
 */
public class BaseForm extends OABase implements Serializable {
    private static Logger LOG = Logger.getLogger(BaseForm.class.getName());
    
    private static final long serialVersionUID = 1L;

    protected final List<BaseComponent> alComponent = new ArrayList();
    protected final List<BaseComponent> alNewAddComponent = new ArrayList();
    ...
    public BaseForm(String id, String url) {
        setId(id);
        setUrl(url);
    }
    ...
    public void reset() {
        for (int i=0; ;i++) {
            ...
            BaseComponent comp = alComponent.get(i);
            comp.reset();
        }
    }
    ...
    public BaseComponent[] getChangedComponents() {
        List<BaseComponent> al = new ArrayList();
        ...
        BaseComponent[] ss = new BaseComponent[al.size()];
        al.toArray(ss);
        return ss;
    }
    ...
    public String getInitScript() {
        ...
        for (int i=0; ;i++) {
            ...
            if (comp instanceof MultipartInterface) {
...
//qqqqqqqqqqqqqqq
    ...
    public BaseComponent getComponent(String id, Class c) {
    ...
    public BaseComponent[] getComponents(String id) {
        if (id == null) return null;
        ...
        BaseComponent[] jcs = al.toArray(new BaseComponent[0]);
        return jcs;
    }

    public void remove(String name) {
        BaseComponent comp = getComponent(name);
        if (comp != null) alComponent.remove(comp);
        super.remove(name);
    }
    public void add(BaseComponent comp) {
        if (comp == null) return;
        add(null, comp);
    }
    public void add(String id, BaseComponent comp) {
        if (comp == null) return;
        if (OAString.isEmpty(id)) id = comp.getId();
        else comp.setId(id);

        if (!OAString.isEmpty(id)) {
            BaseComponent compx = getComponent(id);
    ...
    public String processSubmit(OASession session, HttpServletRequest request, HttpServletResponse response) {
        final OAFormSubmitEvent formSubmitEvent = new OAFormSubmitEvent(this, session, request, response);
    ...
    protected void _processSubmit(final OAFormSubmitEvent formSubmitEvent) {
        ...
        formSubmitEvent.setSubmitComponent(getComponent(id));
        
        // allows components to cancel
        for (BaseComponent comp : alComponent) {
            comp.beforeSubmit(formSubmitEvent);
        }

        // if not cancelled and not a command
        for (BaseComponent comp : alComponent) {
            comp.onSubmit1(formSubmitEvent);
        }

        // if not cancelled and the command submitted form
        for (BaseComponent comp : alComponent) {
            comp.onSubmit2(formSubmitEvent);
        }

        // any cleanup 
        for (BaseComponent comp : alComponent) {
            comp.afterSubmit(formSubmitEvent);
        }
    }
    ...
    protected void processMultipart(ServletRequest request, Map<String, String[]> hmNameValue) throws Exception {
        ...
        for (int i=0;;i++) {
            ...
            */
            ...
            if (!(comp instanceof MultipartInterface)) continue;
            ...
            OutputStream os = ((MultipartInterface)comp).getOutputStream(len, fname);
            if (os == null) {
                os = new OutputStream() {
                    @Override
    ...
    protected void _processForward(final OAFormSubmitEvent formSubmitEvent) {
        ...
        String id = formSubmitEvent.getRequest().getParameter("oacommand");
        final BaseComponent comp = getComponent(id);
        if (comp == null) return;

        formSubmitEvent.setSubmitComponent(comp);

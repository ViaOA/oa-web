...
*/
package com.viaoa.web.ui.bootstrap;
...
/**
 * created 20170521 NOT WORKING qqqqqqqqqqqqq converts an html select using a bootstrap combobox
 * https://github.com/cchantep/bootstrap-combobox requires bootstrap-combobox.js
 * 
 * @author vvia
 * @see OACombo
 */
public class OAComboBox implements OAJspComponent, OATableEditor {
	...
	protected String getAjaxScript(boolean bInit) {
		...
		if (value != null) {
			if (options.length() == 0) {
				for (int i = value.length(); i < columns; i++) {
					value += " ";
				}
		...
		if (getEnabled()) {
			sb.append("$('#" + id + "').btComboBox('enable');\n");
		} else {
			sb.append("$('#" + id + "').btComboBox('disable');\n");
		}
